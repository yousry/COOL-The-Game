...
@implementation YAIntroStage
@synthesize gameContext;

static const NSString* TAG = @"YAIntroStage";

- (id) initWithWorld: (YARenderLoop*) world StateMachine: (YAGameStateMachine*) gameStateMachine
{
    self = [super init];
...
- (void) setupScene
{
   ...
   [self loadModels];
    ...
    soundCollector = stateMachine.soundCollector;
    if(soundCollector == nil) {
    ...
    // copyright
    YAImpersonator* copyrightText = [self genText:@"(c) 2011 - 2016 Yousry Abdallah (www.yousry.de) V 1.22 Free Edition"];
     [[copyrightText rotation] setX: 40];
     [[[copyrightText material] phongAmbientReflectivity] setValues:0.25 :0.1 :0.2];
     copyrightText.material.eta = 0.0f;
     [copyrightText resize: 0.125];
     copyrightText.translation.x -= 3.27;
     copyrightText.translation.z -= 2.5;
