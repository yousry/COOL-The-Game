...
- (id) initInWorld: (YARenderLoop*) world PlayerId: (int) id
{
    ...
    if(self) {
        ...
        int raceId = [_world createImpersonatorWithShapeShifter: @"Spheroid"];
        impersonator = [_world getImpersonator:raceId];
        [impersonator resize:0.3];
        ...
        mover = [[YASpheroidMover alloc] initWithImp:impersonator inWorld:_world];
        ...
        YAVector3f* cylinder = [[YAVector3f alloc]initVals:6.357 :6.357 :7.336];
        [cylinder mulScalar:0.5f];
        [impersonator setCylinderHalfExtents:cylinder];
        
        YAVector3f* offset = [[YAVector3f alloc]initVals:0 :3.69228 :0];
